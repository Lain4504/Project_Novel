      ...
      <form @submit.prevent="handleAddItem" class="space-y-6">
        <div v-for="input in inputs" :key="input.id">
          ...
          <div v-if="input.type === 'select'" class="mt-1">
            <Select v-model:value="formData[input.id]" :id="input.id" class="w-full">
          ...
          <div v-else class="mt-1">
            <Input v-model:value="formData[input.id]" :type="input.type" :id="input.id" :placeholder="input.placeholder || ''" class="w-full" />
          </div>
        </div>
