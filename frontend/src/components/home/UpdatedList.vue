      ...
      <table v-if="!isMobile && novels.length > 0" class="w-full border-collapse">
        <tbody>
          <tr v-for="novel in novels" :key="novel.id" class="border-b hover:bg-gray-50">
            ...
            <td :style="{ width: columnWidths.author + 'px' }" class="px-4 py-2 text-sm text-gray-700">{{ novel.authorName }}</td>
            <td :style="{ width: columnWidths.latestChapter + 'px' }" class="px-4 py-2 text-sm text-gray-700">{{ novel.latestChapterTitle }}</td>
            <td :style="{ width: columnWidths.updatedAt + 'px', textAlign: 'right' }" class="px-4 py-2 text-sm text-gray-500">{{ novel.created }}</td>
          </tr>
        </tbody>
      </table>
